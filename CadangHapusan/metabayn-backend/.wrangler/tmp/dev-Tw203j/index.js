...
// .wrangler/tmp/bundle-pQxOqN/checked-fetch.js
...
// .wrangler/tmp/bundle-pQxOqN/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-pQxOqN/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
async function listVertexModels(env) {
  ...
  const url = `https://${env.location}-aiplatform.googleapis.com/v1/projects/${env.project}/locations/${env.location}/publishers/google/models`;
...
async function handleVertexListModels(_request, env) {
  try {
    const venv = ensureVertexEnv(env);
    const models = await listVertexModels(venv);
    return Response.json({
      ok: true,
      location: venv.location,
      count: models.length,
