...
// .wrangler/tmp/bundle-il6lBZ/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-il6lBZ/middleware-insertion-facade.js
init_modules_watch_stub();
...
async function handleGenerate(req, userId, env) {
  ...
  try {
    ...
    const aiTask = /* @__PURE__ */ __name(async () => {
      ...
      for (const currentModel of fallbackChain) {
        ...
        try {
          ...
          try {
            const waitPromise = waitTurn(currentProvider, void 0, env);
    ...
    if (shouldDeduct) {
      const costFinalUSD2 = calculateTokenCost(userModel, inputTokens, outputTokens, profitMultiplier);
      const currentRate = await getLiveUsdRate(env);
      const costInTokens = costFinalUSD2 * currentRate;
      const deductAmount2 = Math.max(Math.ceil(costInTokens), 1);
      console.log(`[AI] Deducting ${deductAmount2} Tokens (from $${costFinalUSD2.toFixed(6)}) from User ${userId}...`);
      const deductRes2 = await env.DB.prepare("UPDATE users SET tokens = tokens - ? WHERE id = ? RETURNING tokens").bind(deductAmount2, userId).first();
      if (!deductRes2) {
    ...
    const updatedUserTokens = deductRes?.tokens || 0;
    const url = new URL(req.url);
...
async function handleCloudflareGenerate(request, userId, env) {
  ...
  try {
    ...
    const deductRes2 = await env.DB.prepare("UPDATE users SET tokens = tokens - ? WHERE id = ? AND tokens >= ? RETURNING tokens").bind(finalCost, uid, finalCost).first();
    if (!deductRes2) {
      console.error(`[LeakProtection] Failed to deduct tokens for user ${uid}. Balance insufficient or race condition.`);
    } else {
      console.log(`[CloudflareAI] Deducted ${finalCost} tokens for user ${uid}. New balance: ${deductRes2.tokens}`);
    }
    ...
    return new Response(JSON.stringify({
      ...
      user_balance_after: deductRes2?.tokens
      // Return new balance
...
var src_default = {
  ...
  async fetch(request, env, _ctx) {
    ...
    const wrapCors = /* @__PURE__ */ __name(async (response) => {
      try {
      ...
      if ((path === "/v1/models" || path === "/models") && method === "GET") {
        return await wrapCors(Promise.resolve(new Response(JSON.stringify({
          ...
          data: [
            ...
            { id: "openrouter/auto", object: "model", created: 1677610602, owned_by: "openrouter" }
          ]
        }), { headers: corsHeaders })));
      }
