...
// .wrangler/tmp/bundle-rVWZxG/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-rVWZxG/middleware-insertion-facade.js
init_modules_watch_stub();
...
async function waitTurn(provider, key, env) {
  ...
  try {
    if (env) {
      ...
      if (!unlimited && provider === "gemini") {
        const rpmRow = await env.DB.prepare("SELECT value FROM app_config WHERE key = 'gemini_rpm_limit'").first();
        const thrRow = await env.DB.prepare("SELECT value FROM app_config WHERE key = 'gemini_throttle_ms'").first();
        if (rpmRow && rpmRow.value) {
          const n = Number(rpmRow.value);
          if (!isNaN(n) && n > 0) rpmLimit = n;
        }
        if (thrRow && thrRow.value) {
          const n = Number(thrRow.value);
  ...
  if (provider === "gemini" && key) {
    const adaptive = rpmLimits[key];
    if (typeof adaptive === "number" && adaptive > 0) {
      rpmLimit = Math.max(Math.min(adaptive, MAX_RPM), MIN_RPM);
    }
    const start = modelWindowStart[key] || 0;
    const count = modelCount[key] || 0;
    const windowMs = 6e4;
    const now2 = Date.now();
    if (!start || now2 - start >= windowMs) {
      modelWindowStart[key] = now2;
      modelCount[key] = 0;
    }
    if (modelCount[key] >= rpmLimit) {
      const waitMs = windowMs - (now2 - modelWindowStart[key]);
      const jitter2 = 100 + Math.floor(Math.random() * 300);
      await new Promise((r) => setTimeout(r, Math.max(waitMs, 0) + jitter2));
      modelWindowStart[key] = Date.now();
      modelCount[key] = 0;
    }
...
function noteGemini429(key) {
  const now = Date.now();
  last429At[key] = now;
  const current = rpmLimits[key] ?? GEMINI_RPM;
  const next = Math.max(Math.floor(current * 0.7), MIN_RPM);
  rpmLimits[key] = next;
}
__name(noteGemini429, "noteGemini429");
function noteGeminiSuccess(key) {
  const now = Date.now();
  const last = last429At[key] || 0;
  if (!last || now - last > COOLDOWN_MS) {
    const current = rpmLimits[key] ?? GEMINI_RPM;
    const next = Math.min(current + 5, MAX_RPM);
    rpmLimits[key] = next;
  }
}
__name(noteGeminiSuccess, "noteGeminiSuccess");
...
// src/handlers/ai.ts
var VERTEX_ALLOWED_MODELS = /* @__PURE__ */ new Set([
  "gemini-1.0-pro",
...
var CB_STATE = { open: false, failures: 0, lastFailureAt: 0, openedAt: 0 };
var CB_THRESHOLD = 5;
var CB_OPEN_MS = 3e4;
function cbShouldBlock() {
  if (!CB_STATE.open) return false;
  return Date.now() - CB_STATE.openedAt < CB_OPEN_MS;
}
__name(cbShouldBlock, "cbShouldBlock");
function cbRecordFailure() {
  CB_STATE.failures += 1;
  CB_STATE.lastFailureAt = Date.now();
  if (CB_STATE.failures >= CB_THRESHOLD) {
    CB_STATE.open = true;
    CB_STATE.openedAt = Date.now();
  }
}
__name(cbRecordFailure, "cbRecordFailure");
function cbRecordSuccess() {
  CB_STATE.failures = 0;
  CB_STATE.open = false;
}
__name(cbRecordSuccess, "cbRecordSuccess");
function cbReset() {
  CB_STATE = { open: false, failures: 0, lastFailureAt: 0, openedAt: 0 };
}
__name(cbReset, "cbReset");
function normalizePrivateKey(pk) {
  return (pk || "").replace(/\\n/g, "\n").trim();
}
__name(normalizePrivateKey, "normalizePrivateKey");
function ensureVertexEnv(env) {
  ...
  const location = (env.GOOGLE_LOCATION || "us-central1").trim();
  if (!project || !email || !key) throw new Error("VertexEnvInvalid");
  return { project, email, key, location };
}
__name(ensureVertexEnv, "ensureVertexEnv");
function normalizeVertexModel(model) {
  const m = (model || "").trim().toLowerCase();
  if (m === "gemini-flash") return "gemini-1.5-flash";
  if (m.includes("flash-lite")) return "gemini-2.5-flash";
  if (m === "gemini-1.5-flash-002") return "gemini-1.5-flash";
  if (m === "gemini-1.5-pro-002") return "gemini-1.5-pro";
  if (m === "gemini-2.0-flash-exp") return "gemini-2.0-flash";
  if (m === "gemini-2.0-pro-exp-02-05") return "gemini-2.0-pro";
  if (m === "gemini-2.0-flash-lite-preview-02-05") return "gemini-2.5-flash";
  if (m === "gemini-2.5-ultra") return "gemini-2.5-pro";
  if (m === "gemini-3.0-ultra") return "gemini-2.5-pro";
  return model;
}
__name(normalizeVertexModel, "normalizeVertexModel");
var MODEL_CACHE = null;
async function listVertexModels(env) {
  if (MODEL_CACHE && Date.now() - MODEL_CACHE.fetchedAt < 3e5) return MODEL_CACHE.list;
  const accessToken = await getGoogleAccessToken(env.email, env.key);
  const urls = [
    `https://${env.location}-aiplatform.googleapis.com/v1/projects/${env.project}/locations/${env.location}/publishers/google/models`,
    `https://${env.location}-aiplatform.googleapis.com/v1beta1/projects/${env.project}/locations/${env.location}/publishers/google/models`
  ];
  let data = null;
  let lastErr = null;
  for (const u of urls) {
  ...
  if (!data) throw new Error(`NON_RETRYABLE: ${lastErr}`);
  const names = Array.isArray(data.models) ? data.models.map((m) => (m.name || "").split("/").pop()).filter(Boolean) : [];
  MODEL_CACHE = { list: names, fetchedAt: Date.now() };
  return names;
}
__name(listVertexModels, "listVertexModels");
async function vertexGenerate(env, modelId, parts, timeoutMs, retries) {
  if (cbShouldBlock()) throw new Error("CircuitBreakerOpen");
  const accessToken = await getGoogleAccessToken(env.email, env.key);
  const url = `https://${env.location}-aiplatform.googleapis.com/v1/projects/${env.project}/locations/${env.location}/publishers/google/models/${modelId}:generateContent`;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status >= 500 || res.status === 429) {
          ...
          if (res.status === 429) {
            try {
              noteGemini429(modelId);
            } catch {
            }
          }
          if (attempt < retries) {
            const ra = res.headers.get("retry-after");
            const base = ra ? Math.max(Number(ra) * 1e3, 500) : 700;
        ...
        throw new Error(`NON_RETRYABLE: [${res.status}] ${txt}`);
      }
      const data = await res.json();
      cbRecordSuccess();
      try {
        noteGeminiSuccess(modelId);
      } catch {
      }
      return data;
    } catch (e) {
      ...
      if (attempt < retries) {
        const jitter = 100 + Math.floor(Math.random() * 300);
        await new Promise((r) => setTimeout(r, Math.pow(2, attempt) * 700 + jitter));
        continue;
      }
      throw e;
    }
  }
  throw new Error("VertexRetryExhausted");
}
__name(vertexGenerate, "vertexGenerate");
async function handleGenerate(req, userId, env) {
  ...
  try {
    ...
    try {
      ...
      let limit = Number(concRow?.value) || 5;
    ...
    const aiTask = /* @__PURE__ */ __name(async () => {
      ...
      for (const currentModel of fallbackChain) {
        ...
        try {
          ...
          await waitTurn(currentProvider, currentProvider === "gemini" ? normalizeVertexModel(currentModel) : void 0, env);
          let content2 = "";
          let inputTokens2 = 0;
          let outputTokens2 = 0;
          ...
          if (currentProvider === "openai") {
            ...
            let venv = ensureVertexEnv(env);
            try {
              const locRow = await env.DB.prepare("SELECT value FROM app_config WHERE key = 'vertex_default_location'").first();
              const loc = String(locRow?.value || "").trim();
              if (loc) venv = { ...venv, location: loc };
            } catch {
            }
            let targetModel = normalizeVertexModel(currentModel);
            ...
            try {
              ...
              const available = await listVertexModels(venv).catch(() => []);
              if (!available.includes(vertexModelId) || !VERTEX_ALLOWED_MODELS.has(vertexModelId)) {
                const priority = ["gemini-2.5-flash", "gemini-2.5-pro", "gemini-2.0-flash", "gemini-1.5-flash-8b", "gemini-1.5-flash"];
                vertexModelId = priority.find((m) => available.includes(m)) || "gemini-2.5-flash";
              }
              let data = await vertexGenerate(venv, vertexModelId, parts, 12e3, 3);
              ...
              if ((e.message || "").includes("[404]") || (e.message || "").includes("NOT_FOUND") || (e.message || "").includes("Publisher Model")) {
                let alt = "gemini-2.5-flash";
                try {
                  const available = await listVertexModels(venv).catch(() => []);
                  const priority = ["gemini-2.5-flash", "gemini-2.5-pro", "gemini-2.0-flash", "gemini-1.5-flash-8b", "gemini-1.5-flash"];
                  alt = priority.find((m) => available.includes(m)) || alt;
                  const data2 = await vertexGenerate(venv, alt, parts, 12e3, 2);
                  const cand2 = data2.candidates?.[0];
                  if (!cand2 || !cand2.content?.parts?.[0]?.text) throw new Error("Empty response from AI");
                  content2 = cand2.content.parts[0].text;
                  inputTokens2 = data2.usageMetadata?.promptTokenCount || Math.ceil((prompt || "").length / 4);
                  outputTokens2 = data2.usageMetadata?.candidatesTokenCount || Math.ceil(content2.length / 4);
                } catch (e2) {
                  throw e2;
                }
              } else {
...
async function handleVertexEnvCheck(_request, env) {
  try {
    const venv = ensureVertexEnv(env);
    return Response.json({
      ok: true,
      project: venv.project,
      client_email: venv.email,
...
__name(handleVertexEnvCheck, "handleVertexEnvCheck");
async function handleVertexListModels(_request, env) {
  try {
    const venv = ensureVertexEnv(env);
    const url = new URL(_request.url);
    const loc = (url.searchParams.get("location") || "").trim();
    const v2 = loc ? { ...venv, location: loc } : venv;
    const models = await listVertexModels(v2);
    return Response.json({
      ok: true,
...
__name(handleVertexListModels, "handleVertexListModels");
async function handleVertexTestGenerate(request, env) {
  try {
    const venv = ensureVertexEnv(env);
    const url = new URL(request.url);
    const loc = (url.searchParams.get("location") || "").trim();
    const v2 = loc ? { ...venv, location: loc } : venv;
    const model = normalizeVertexModel(url.searchParams.get("model") || "gemini-2.5-flash");
    const prompt = url.searchParams.get("prompt") || "Say hello";
    const parts = [{ text: prompt }];
    const data = await vertexGenerate(v2, model, parts, 12e3, 1);
    const text = data?.candidates?.[0]?.content?.parts?.[0]?.text || "";
    return Response.json({ ok: true, location: v2.location, model, text });
  } catch (e) {
    return Response.json({ ok: false, error: e.message }, { status: 400 });
  }
}
__name(handleVertexTestGenerate, "handleVertexTestGenerate");
async function handleVertexTestVision(request, env) {
  try {
    const venv = ensureVertexEnv(env);
    const url = new URL(request.url);
    const loc = (url.searchParams.get("location") || "").trim();
    const v2 = loc ? { ...venv, location: loc } : venv;
    const model = normalizeVertexModel(url.searchParams.get("model") || "gemini-2.5-flash");
    const prompt = url.searchParams.get("prompt") || "Describe the image";
    const pngBase64 = "";
    const parts = [{ text: prompt }, { inline_data: { mime_type: "image/png", data: pngBase64 } }];
    const data = await vertexGenerate(v2, model, parts, 12e3, 1);
    const text = data?.candidates?.[0]?.content?.parts?.[0]?.text || "";
    return Response.json({ ok: true, location: v2.location, model, text });
  } catch (e) {
...
__name(handleVertexTestVision, "handleVertexTestVision");
async function handleVertexTestVisionPost(request, env) {
  try {
    const venv = ensureVertexEnv(env);
    const body = await request.json();
    const loc = (body.location || "").trim();
    const v2 = loc ? { ...venv, location: loc } : venv;
    const model = normalizeVertexModel(body.model || "gemini-2.5-flash");
...
__name(handleVertexTestVisionPost, "handleVertexTestVisionPost");
...
async function handleCloudflareGenerate(request, userId, env) {
  ...
  const user = await env.DB.prepare("SELECT tokens FROM users WHERE id = ?").bind(uid).first();
  if (!user || user.tokens <= 0) {
    return new Response(JSON.stringify({ error: "Insufficient balance. Please Top Up." }), { status: 402 });
  }
    ...
    if (validate_json) {
      ...
      try {
        ...
        if (start !== -1 && end > start) {
          try {
            const potentialJson = resultText.substring(start, end + 1);
      ...
      if (!isValidJson) {
        console.error(`[LeakProtection] AI returned invalid JSON when validate_json=true: ${resultText.substring(0, 100)}...`);
        return new Response(JSON.stringify({ error: `AI Response is not valid JSON: ${resultText.substring(0, 100)}. No tokens deducted.` }), { status: 422 });
      }
    }
    const inputTokens = Math.ceil(prompt.length / 4);
    const outputTokens = Math.ceil(resultText.length / 4);
    const cost = await calculateTokenCost(inputTokens, outputTokens, finalModel, env);
    const deductRes = await env.DB.prepare("UPDATE users SET tokens = tokens - ? WHERE id = ? AND tokens >= ? RETURNING tokens").bind(cost, uid, cost).first();
    if (!deductRes) {
      console.error(`[LeakProtection] Failed to deduct tokens for user ${uid}. Balance insufficient or race condition.`);
    } else {
      console.log(`[CloudflareAI] Deducted ${cost} tokens for user ${uid}. New balance: ${deductRes.tokens}`);
    }
    await recordTokenUsage(uid, finalModel, finalModel, inputTokens, outputTokens, cost, env);
  ...
  async fetch(request, env, _ctx) {
    ...
    try {
      ...
      if (path.startsWith("/admin/")) {
        ...
        if (path === "/admin/debug/vertex/env" && method === "GET") return await wrapCors(handleVertexEnvCheck(request, env));
        if (path === "/admin/debug/vertex/models" && method === "GET") return await wrapCors(handleVertexListModels(request, env));
        if (path === "/admin/debug/vertex/test" && method === "GET") return await wrapCors(handleVertexTestGenerate(request, env));
        if (path === "/admin/debug/vertex/test-vision" && method === "GET") return await wrapCors(handleVertexTestVision(request, env));
        if (path === "/admin/debug/vertex/test-vision" && method === "POST") return await wrapCors(handleVertexTestVisionPost(request, env));
        if (path === "/admin/debug/vertex/cb-reset" && method === "POST") {
          cbReset();
          return await wrapCors(Promise.resolve(new Response(JSON.stringify({ ok: true }), { headers: corsHeaders })));
        }
      ...
      if (path === "/debug/vertex/env" && method === "GET") return await wrapCors(handleVertexEnvCheck(request, env));
      if (path === "/debug/vertex/models" && method === "GET") return await wrapCors(handleVertexListModels(request, env));
