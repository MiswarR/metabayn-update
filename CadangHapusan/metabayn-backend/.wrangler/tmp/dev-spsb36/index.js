...
// .wrangler/tmp/bundle-KmFGZJ/checked-fetch.js
...
// .wrangler/tmp/bundle-KmFGZJ/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-KmFGZJ/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
async function waitTurn(provider) {
  ...
  let proceed = /* @__PURE__ */ __name(() => {
  }, "proceed");
  const gate = new Promise((r) => {
    proceed = r;
  });
  chains[provider] = prev.then(async () => {
...
async function calculateTokenCost(inputTokens, outputTokens, userModel, env) {
  ...
  if (!price) {
    if (userModel.includes("flash-lite")) price = SAFE_PRICES["gemini-2.0-flash-lite"];
    else if (userModel.includes("8b")) price = SAFE_PRICES["gemini-1.5-flash-8b"];
...
async function handleGenerate(req, userId, env) {
  ...
  try {
    ...
    let fallbackChain = [userModel];
    if (userModel.startsWith("gemini")) {
      const m = userModel;
      if (m === "gemini-2.0-flash-lite" || m === "gemini-2.0-flash-lite-preview-02-05") {
        fallbackChain = ["gemini-2.5-flash-lite", "gemini-2.5-flash"];
      } else if (m === "gemini-flash") {
        fallbackChain = ["gemini-2.5-flash", "gemini-2.5-flash-lite"];
      } else if (m === "gemini-2.0-flash") {
        fallbackChain = ["gemini-2.5-flash", "gemini-2.5-flash-lite"];
      } else if (m === "gemini-2.0-pro" || m === "gemini-2.0-pro-exp-02-05") {
        fallbackChain = ["gemini-2.5-pro", "gemini-2.5-flash"];
      } else if (m.startsWith("gemini-3.0")) {
        fallbackChain = ["gemini-2.5-flash", "gemini-2.5-flash-lite"];
      } else if (m === "gemini-2.5-flash") {
        fallbackChain = ["gemini-2.5-flash", "gemini-2.5-flash-lite"];
      } else if (m === "gemini-2.5-flash-lite") {
        fallbackChain = ["gemini-2.5-flash-lite", "gemini-2.5-flash"];
      } else {
        fallbackChain = [m, "gemini-2.5-flash-lite", "gemini-2.5-flash"];
      }
    }
      ...
      for (const currentModel of fallbackChain) {
        ...
        try {
          ...
          if (currentProvider === "openai") {
            ...
            if (targetModel === "gemini-2.0-flash-lite" || targetModel === "gemini-2.0-flash-lite-preview-02-05") {
              targetModel = "gemini-2.5-flash-lite";
            } else if (targetModel === "gemini-flash") {
              targetModel = "gemini-2.5-flash";
            } else if (targetModel === "gemini-2.0-flash") {
              targetModel = "gemini-2.5-flash";
            } else if (targetModel === "gemini-2.0-pro" || targetModel === "gemini-2.0-pro-exp-02-05") {
              targetModel = "gemini-2.5-pro";
            } else if (targetModel.startsWith("gemini-3.0")) {
              targetModel = "gemini-2.5-flash";
            }
            ...
            try {
              ...
              if (useVertex) {
                try {
                  ...
                  if (res.status === 429 || res.status === 503) {
                    ...
                    const timeoutId2 = setTimeout(() => controller2.abort(), 2e4);
                    res = await fetch(vertexUrl, {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json",
                  ...
                  console.warn(`[AI] Vertex AI Failed (falling back to AI Studio): ${e.message}`);
                  vertexSuccess = false;
                }
              }
              if (!vertexSuccess) {
                if (!env.GEMINI_API_KEY) {
                  throw new Error("NON_RETRYABLE: Both Vertex AI and Gemini API Key are missing/failed.");
                }
                console.log("Using Legacy Gemini API (AI Studio)");
              ...
              if (data.usageMetadata) {
                inputTokens2 = data.usageMetadata.promptTokenCount;
                outputTokens2 = data.usageMetadata.candidatesTokenCount;
              } else {
                const textLen = (prompt || "").length;
                const textTokens = Math.ceil(textLen / 4);
                const hasImage = !!image || !!body.image || body.contents?.[0]?.parts?.some((p) => p.inline_data || p.image_url);
                const imageTokens = hasImage ? 258 : 0;
                inputTokens2 = textTokens + imageTokens;
                outputTokens2 = Math.ceil(content2.length / 4);
                console.warn(`[AI] Missing usage metadata from Provider. Using fallback: In=${inputTokens2} (Text:${textTokens} + Img:${imageTokens}), Out=${outputTokens2}`);
              }
            } catch (e) {
...
var src_default = {
  ...
  async fetch(request, env, _ctx) {
    ...
    try {
      ...
      if (path === "/config/models" && method === "GET") return await wrapCors(handleGetConfigModels(request, env));
