...
async function validateUserAccess(userId, env, options = {}) {
  ...
  if (!isSubscriptionValid) {
    return {
      valid: false,
      error: "Generate metadata diblokir \u2013 user Free",
      status: 403,
      user
    };
  }
...
function parsePercentToBps(percentInput) {
  ...
  const sign2 = s.startsWith("-") ? -1n : 1n;
  ...
  return sign2 * (intPart * 100n + fracPart);
}
...
function formatTenthsToString(tenths) {
  ...
  const dec = abs % 10n;
  return `${sign2}${intPart.toString()}.${dec.toString()}`;
}
function computeIdrChargeTenthsFromUsdCost({
  costUsd,
  profitMarginPercent,
  usdIdrRate
}) {
...
async function insertDoubleEntry(env, { txId, userId, userDeltaTenths, systemAccount, systemDeltaTenths }) {
  ...
  ).bind(txId, uid, Number(userDeltaTenths), Date.now()).run();
  await env.DB.prepare(
    "INSERT INTO balance_entries (tx_id, account_type, account_id, delta_tenths, created_at) VALUES (?, 'system', ?, ?, ?)"
  ).bind(txId, sys, Number(systemDeltaTenths), Date.now()).run();
}
async function chargeUserBalanceFromUsdCost(env, opts) {
  const {
    userId,
    costUsd,
    reason,
  ...
  const existing = await getExistingTransactionByIdempotencyKey(env, idempotencyKey);
  if (existing && existing.id) {
    ...
    return {
      ...
      error: existing.error ? String(existing.error) : void 0,
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      const beforeRow = await env.DB.prepare("SELECT CAST(ROUND(tokens * 10, 0) AS INTEGER) AS tenths FROM users WHERE id = ? LIMIT 1").bind(userId).first();
      const beforeTenths = BigInt(String(beforeRow?.tenths ?? 0));
      if (beforeTenths < costTenths) {
        const msg3 = insufficientLogMessage || `Saldo token tidak cukup: dibutuhkan Rp ${costStr}, tersisa Rp ${formatTenthsToString(beforeTenths)}`;
        await env.DB.prepare("ROLLBACK").run();
        await writeActivityLog(env, { userId, level: "ERROR", message: msg3, timestampMs: ts });
        try {
          await env.DB.prepare(
            ...
            txId,
            toTrimmedString(idempotencyKey) || null,
            String(userId),
            "charge",
            Number(costTenths),
            Number(beforeTenths),
        ...
        return {
          ok: false,
          status: 402,
          error: insufficientErrorMessage || msg3,
      ...
      const updateRes = await env.DB.prepare(
        ...
        `
      ).bind(Number(costTenths), userId, Number(beforeTenths)).first();
      if (!updateRes) {
        await env.DB.prepare("ROLLBACK").run();
        continue;
      }
      ...
      const msg2 = `Potong saldo Rp ${costStr} \u2013 saldo menjadi Rp ${formatTenthsToString(afterTenths)}${reason ? ` (${reason})` : ""}`;
      await writeActivityLog(env, { userId, level: "INFO", message: msg2, timestampMs: ts });
      ...
      await env.DB.prepare("ROLLBACK").run();
      const msg2 = `Gagal potong saldo: ${String(e?.message || e)}`;
      await writeActivityLog(env, { userId, level: "ERROR", message: msg2, timestampMs: ts });
      try {
        await env.DB.prepare(
          "INSERT INTO balance_transactions (id, idempotency_key, user_id, type, amount_tenths, balance_before_tenths, balance_after_tenths, status, error, meta, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        ).bind(
  ...
  await writeActivityLog(env, { userId, level: "ERROR", message: msg, timestampMs: ts });
  return { ok: false, status: 409, error: msg, tokensDeductedTenths: costTenths, userBalanceAfterTenths: 0n };
}
async function addUserBalanceTenths(env, opts) {
    ...
    try {
      ...
      const updateRes = await env.DB.prepare(
        `
          ...
          RETURNING CAST(ROUND(tokens * 10, 0) AS INTEGER) AS tenths
        `
      ).bind(Number(addTenths), userId, Number(beforeTenths)).first();
      if (!updateRes) {
        await env.DB.prepare("ROLLBACK").run();
        continue;
      }
      ...
      const msg2 = `${label} Rp ${formatTenthsToString(addTenths)} \u2013 saldo menjadi Rp ${formatTenthsToString(afterTenths)}${reason ? ` (${reason})` : ""}`;
      await writeActivityLog(env, { userId, level: "INFO", message: msg2, timestampMs: ts });
      await env.DB.prepare("COMMIT").run();
      return { ok: true, userBalanceAfterTenths: afterTenths, txId };
    } catch (e) {
      await env.DB.prepare("ROLLBACK").run();
      const msg2 = `Gagal tambah saldo: ${String(e?.message || e)}`;
      await writeActivityLog(env, { userId, level: "ERROR", message: msg2, timestampMs: ts });
      try {
        await env.DB.prepare(
          "INSERT INTO balance_transactions (id, idempotency_key, user_id, type, amount_tenths, balance_before_tenths, balance_after_tenths, status, error, meta, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
...
async function reverseBalanceTransaction(env, opts) {
  ...
  const original = await env.DB.prepare("SELECT id, user_id, type, amount_tenths, status FROM balance_transactions WHERE id = ? LIMIT 1").bind(id).first();
  ...
  try {
    ...
    const updateRes = await env.DB.prepare(
      `
        ...
        RETURNING CAST(ROUND(tokens * 10, 0) AS INTEGER) AS tenths
      `
    ).bind(Number(amount), userId, Number(beforeTenths)).first();
    if (!updateRes) {
      await env.DB.prepare("ROLLBACK").run();
    ...
    await env.DB.prepare("COMMIT").run();
    try {
      await env.DB.prepare("INSERT INTO admin_logs (admin_id, action, target_id, created_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)").bind(String(adminId || ""), "balance_reverse", String(id)).run();
    } catch {
    }
...
var USD_SCALE, USD_SCALE_FACTOR, IDR_TENTHS_DIVISOR, IDR_TENTHS_ROUND_ADD;
var init_balanceLedger = __esm({
  "src/utils/balanceLedger.js"() {
    "use strict";
    init_modules_watch_stub();
    USD_SCALE = 12n;
    USD_SCALE_FACTOR = 10n ** USD_SCALE;
    IDR_TENTHS_DIVISOR = 10n ** 11n;
    IDR_TENTHS_ROUND_ADD = 5n * 10n ** 10n;
    __name(toTrimmedString, "toTrimmedString");
    __name(normalizeDecimalString, "normalizeDecimalString");
    __name(parsePercentToBps, "parsePercentToBps");
    __name(parseUsdToScaled, "parseUsdToScaled");
    __name(formatTenthsToString, "formatTenthsToString");
    __name(computeIdrChargeTenthsFromUsdCost, "computeIdrChargeTenthsFromUsdCost");
    __name(getAdminPricingConfig, "getAdminPricingConfig");
    __name(writeActivityLog, "writeActivityLog");
    __name(createTxId, "createTxId");
    __name(getExistingTransactionByIdempotencyKey, "getExistingTransactionByIdempotencyKey");
    __name(insertDoubleEntry, "insertDoubleEntry");
    __name(chargeUserBalanceFromUsdCost, "chargeUserBalanceFromUsdCost");
    __name(addUserBalanceTenths, "addUserBalanceTenths");
    __name(reverseBalanceTransaction, "reverseBalanceTransaction");
  }
});
...
async function handleSyncModelPrices(request, env) {
  try {
    ...
    const OFFICIAL = [
      { provider: "openrouter", model_name: "openrouter/free", input_price: 6406e-7, output_price: 6406e-7, active: 1, fallback_priority: 1 },
      { provider: "openrouter", model_name: "qwen/qwen3-vl-235b-a22b-thinking", input_price: 6406e-7, output_price: 6406e-7, active: 1, fallback_priority: 1 },
      { provider: "openrouter", model_name: "qwen/qwen3-vl-30b-a3b-thinking", input_price: 6406e-7, output_price: 6406e-7, active: 1, fallback_priority: 1 },
      { provider: "openrouter", model_name: "nvidia/nemotron-nano-12b-v2-vl:free", input_price: 6406e-7, output_price: 6406e-7, active: 1, fallback_priority: 1 },
      { provider: "openrouter", model_name: "mistralai/mistral-small-3.1-24b-instruct:free", input_price: 6406e-7, output_price: 6406e-7, active: 1, fallback_priority: 1 },
      { provider: "openrouter", model_name: "google/gemma-3-27b-it:free", input_price: 6406e-7, output_price: 6406e-7, active: 1, fallback_priority: 1 },
...
async function handleSyncLiveModelPrices(request, env) {
  try {
    ...
    let count = 0;
    try {
      await upsert("openrouter", "openrouter/free", 6406e-7, 6406e-7);
      await upsert("openrouter", "qwen/qwen3-vl-235b-a22b-thinking", 6406e-7, 6406e-7);
      await upsert("openrouter", "qwen/qwen3-vl-30b-a3b-thinking", 6406e-7, 6406e-7);
      await upsert("openrouter", "nvidia/nemotron-nano-12b-v2-vl:free", 6406e-7, 6406e-7);
      await upsert("openrouter", "mistralai/mistral-small-3.1-24b-instruct:free", 6406e-7, 6406e-7);
      await upsert("openrouter", "google/gemma-3-27b-it:free", 6406e-7, 6406e-7);
    } catch {
    }
    try {
    ...
    if (count === 0) {
      const fallback = [
        ["openrouter", "openrouter/free", 6406e-7, 6406e-7],
        ["openrouter", "qwen/qwen3-vl-235b-a22b-thinking", 6406e-7, 6406e-7],
        ["openrouter", "qwen/qwen3-vl-30b-a3b-thinking", 6406e-7, 6406e-7],
        ["openrouter", "nvidia/nemotron-nano-12b-v2-vl:free", 6406e-7, 6406e-7],
        ["openrouter", "mistralai/mistral-small-3.1-24b-instruct:free", 6406e-7, 6406e-7],
        ["openrouter", "google/gemma-3-27b-it:free", 6406e-7, 6406e-7],
        ["openai", "gpt-4o", 5, 15],
...
async function addUserTokens(userId, tokens, env, options = {}) {
...
// .wrangler/tmp/bundle-3kIi92/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-3kIi92/middleware-insertion-facade.js
init_modules_watch_stub();
...
var OPENROUTER_FREE_PRICE_PER_1M = {
  input: 6406e-7,
  output: 6406e-7
};
var OPENROUTER_FREE_MODEL_IDS = /* @__PURE__ */ new Set([
  "qwen/qwen3-vl-235b-a22b-thinking",
  "qwen/qwen3-vl-30b-a3b-thinking"
]);
var OPENROUTER_PRICING_TTL_MS = 6 * 60 * 60 * 1e3;
...
__name(isOpenRouterFreeModel, "isOpenRouterFreeModel");
async function getOpenRouterPricingMap(env) {
  const now = Date.now();
  if (openRouterPricingCache && now - openRouterPricingCache.ts < OPENROUTER_PRICING_TTL_MS) {
    return openRouterPricingCache.map;
  }
  const apiKey = env?.OPENROUTER_API_KEY || env?.OPENROUTER_MANAGEMENT_KEY || env?.OPENROUTER_KEY;
  const map = /* @__PURE__ */ new Map();
  if (!isNonEmptyString(apiKey)) {
    openRouterPricingCache = { ts: now, map };
    return map;
  }
  try {
...
__name(getOpenRouterPricingMap, "getOpenRouterPricingMap");
async function calculateOpenRouterCostUsd(userModel, actualModelUsed, inputTokens, outputTokens, env, hasImage) {
  const usedModel = String(actualModelUsed || userModel || "").trim();
  const isFree = isOpenRouterFreeModel(usedModel);
  const userWantsGlobalFreePrice = String(userModel || "").trim() === "openrouter/free";
  let rawCostUsd = 0;
  if (isFree) {
    let dbPrice = null;
    const usedModelForDb = usedModel.endsWith(":free") && usedModel !== "openrouter/free" ? usedModel.slice(0, -":free".length) : usedModel;
    try {
      const globalRow = userWantsGlobalFreePrice ? await env.DB.prepare(
        "SELECT input_price, output_price FROM model_prices WHERE provider = 'openrouter' AND model_name = 'openrouter/free' AND active = 1 ORDER BY fallback_priority ASC LIMIT 1"
      ).first() : null;
      const row = globalRow || await env.DB.prepare(
        "SELECT input_price, output_price FROM model_prices WHERE provider = 'openrouter' AND model_name = ? AND active = 1 ORDER BY fallback_priority ASC LIMIT 1"
      ).bind(usedModel).first() || (usedModelForDb !== usedModel ? await env.DB.prepare(
        "SELECT input_price, output_price FROM model_prices WHERE provider = 'openrouter' AND model_name = ? AND active = 1 ORDER BY fallback_priority ASC LIMIT 1"
      ).bind(usedModelForDb).first() : null) || await env.DB.prepare(
        "SELECT input_price, output_price FROM model_prices WHERE provider = 'openrouter' AND model_name = 'openrouter/free' AND active = 1 ORDER BY fallback_priority ASC LIMIT 1"
      ).first();
      if (row && Number.isFinite(Number(row.input_price)) && Number.isFinite(Number(row.output_price))) {
        dbPrice = { input: Number(row.input_price), output: Number(row.output_price) };
      }
    } catch {
    }
    const base = dbPrice || OPENROUTER_FREE_PRICE_PER_1M;
    rawCostUsd = inputTokens / 1e6 * base.input + outputTokens / 1e6 * base.output;
  } else {
    const pricingMap = await getOpenRouterPricingMap(env);
    const p = pricingMap.get(usedModel);
    if (p && (p.promptPerToken > 0 || p.completionPerToken > 0 || p.requestPerRequest > 0 || p.imagePerImage > 0)) {
      rawCostUsd = inputTokens * p.promptPerToken + outputTokens * p.completionPerToken + p.requestPerRequest + (hasImage ? p.imagePerImage : 0);
    } else {
      let price = SAFE_PRICES[usedModel];
      if (!price) {
        if (usedModel.includes("flash")) price = SAFE_PRICES["gemini-1.5-flash"];
        else if (usedModel.includes("pro")) price = SAFE_PRICES["gemini-1.5-pro"];
        else price = SAFE_PRICES["default"];
      }
      rawCostUsd = inputTokens / 1e6 * price.input + outputTokens / 1e6 * price.output;
    }
  }
  return rawCostUsd;
}
__name(calculateOpenRouterCostUsd, "calculateOpenRouterCostUsd");
...
async function recordTokenUsage(userId, userModel, actualModelUsed, inputTokens, outputTokens, cost, env) {
  try {
    await env.DB.prepare("INSERT INTO history (user_id, model, input_tokens, output_tokens, cost, timestamp, actual_model_used) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(userId, userModel, inputTokens, outputTokens, cost, Date.now(), actualModelUsed).run();
  } catch (e) {
    try {
      await env.DB.prepare("INSERT INTO history (user_id, model, input_tokens, output_tokens, cost, actual_model_used) VALUES (?, ?, ?, ?, ?, ?)").bind(userId, userModel, inputTokens, outputTokens, cost, actualModelUsed).run();
    } catch (e2) {
      try {
...
init_validation();
init_balanceLedger();
var OPENROUTER_FREE_PRICE_SYNC_TTL_MS = 6 * 60 * 60 * 1e3;
var openRouterFreePriceLastSyncTs = 0;
async function ensureOpenRouterFreePrices(env) {
  const now = Date.now();
  if (openRouterFreePriceLastSyncTs && now - openRouterFreePriceLastSyncTs < OPENROUTER_FREE_PRICE_SYNC_TTL_MS) return;
  openRouterFreePriceLastSyncTs = now;
  const inputPrice = 6406e-7;
  const outputPrice = 6406e-7;
  const models = [
    "openrouter/free",
    "qwen/qwen3-vl-235b-a22b-thinking",
    "qwen/qwen3-vl-30b-a3b-thinking",
  ...
  const upsert = /* @__PURE__ */ __name(async (model_name) => {
    const existing = await env.DB.prepare("SELECT id FROM model_prices WHERE model_name = ?").bind(model_name).first();
    if (existing && existing.id) {
      await env.DB.prepare(`
        UPDATE model_prices
        SET provider = ?, input_price = ?, output_price = ?, profit_multiplier = ?, active = ?, fallback_priority = ?
        WHERE id = ?
      `).bind("openrouter", inputPrice, outputPrice, 1, 1, 1, existing.id).run();
    } else {
      await env.DB.prepare(`
        INSERT INTO model_prices (provider, model_name, input_price, output_price, profit_multiplier, active, fallback_priority)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).bind("openrouter", model_name, inputPrice, outputPrice, 1, 1, 1).run();
    }
  }, "upsert");
  try {
...
__name(ensureOpenRouterFreePrices, "ensureOpenRouterFreePrices");
async function handleGenerate(req, userId, env) {
  ...
  try {
    ...
    const connectionModeRaw = String(body?.connection_mode || body?.connectionMode || "").trim().toLowerCase();
    ...
    let feature = body?.feature === "csv_fix" ? "csv_fix" : "metadata";
    ...
    const extractMessageText = /* @__PURE__ */ __name((msgs) => {
    ...
    const estimateTokensFromText = /* @__PURE__ */ __name((text) => {
      const t = String(text || "");
      const approx = Math.ceil(t.length / 4);
      const n = Number.isFinite(approx) ? approx : 0;
      return Math.max(1, Math.min(n, 2e4));
    ...
    const aiTask = /* @__PURE__ */ __name(async () => {
      ...
      for (const currentModel of fallbackChain) {
        ...
        try {
          ...
          let inputTokens2 = 0;
          let outputTokens2 = 0;
          let costUsdFromOpenRouter2 = void 0;
          let costUsdSource2 = "usage_pricing";
          ...
          try {
            ...
            if (costHeaderRaw !== null) {
              ...
              if (Number.isFinite(parsed) && parsed > 0) {
                costUsdFromOpenRouter2 = parsed;
                costUsdSource2 = "header";
              }
            }
            ...
            const openRouterGenerationId2 = data?.id ? String(data.id) : "";
            if (costUsdFromOpenRouter2 === void 0) {
              const generationId = openRouterGenerationId2;
              if (generationId) {
                try {
          ...
          return { content: content2, inputTokens: inputTokens2, outputTokens: outputTokens2, usedModel: currentModel, costUsdFromOpenRouter: costUsdFromOpenRouter2, costUsdSource: costUsdSource2, openRouterGenerationId };
        } catch (e) {
    ...
    const { content, inputTokens, outputTokens, usedModel, costUsdFromOpenRouter, costUsdSource, openRouterGenerationId } = result;
    let promptTokensFinal = Number(inputTokens || 0);
    let completionTokensFinal = Number(outputTokens || 0);
    const isStandardMode = connectionModeRaw === "direct" && !!user.or_api_key;
    const shouldDeduct = feature === "csv_fix" || !isStandardMode;
    let costFinalUSD = 0;
    let deductAmount = 0;
    let updatedUserTokens = Number(user?.tokens ?? 0) || 0;
    const costUsdSourceFinal = shouldDeduct ? typeof costUsdFromOpenRouter === "number" && Number.isFinite(costUsdFromOpenRouter) && costUsdFromOpenRouter > 0 ? costUsdSource : "usage_pricing" : "none";
    if (shouldDeduct) {
      if (typeof costUsdFromOpenRouter === "number" && Number.isFinite(costUsdFromOpenRouter) && costUsdFromOpenRouter > 0) {
        ...
        await ensureOpenRouterFreePrices(env);
        const hasImage = !!image || !!mimeType || Array.isArray(messages) && messages.some((m) => {
          const c = m?.content;
          if (Array.isArray(c)) {
            return c.some((p) => !!p?.image_url || p?.type === "image_url" || p?.type === "input_image");
          }
        ...
        if (promptTokensFinal + completionTokensFinal <= 0) {
          const baseText = typeof prompt === "string" && prompt.trim().length > 0 ? prompt : extractMessageText(messages);
          promptTokensFinal = estimateTokensFromText(String(baseText).slice(0, 2e4));
          completionTokensFinal = estimateTokensFromText(String(content || "").slice(0, 2e4));
        }
      ...
      if (!(typeof costFinalUSD === "number" && Number.isFinite(costFinalUSD)) || costFinalUSD <= 0) {
        try {
          const { profitMarginPercent, usdIdrRate } = await getAdminPricingConfig(env);
          const profit = Number(String(profitMarginPercent).replace(",", "."));
          const multiplier = 1 + Math.max(0, Number.isFinite(profit) ? profit : 0) / 100;
          const rate = Number(usdIdrRate);
          if (Number.isFinite(rate) && rate > 0 && Number.isFinite(multiplier) && multiplier > 0) {
            const minUsd = 0.05 / (rate * multiplier);
      ...
      const insufficientLogMessage = feature === "csv_fix" && !subscriptionValid ? "Generate CSV metadata Free user dibatalkan \u2013 saldo token kurang" : void 0;
      ...
      const charge = await chargeUserBalanceFromUsdCost(env, {
        ...
        insufficientLogMessage,
        insufficientErrorMessage: void 0,
        idempotencyKey,
        meta: {
          kind: "ai_generate",
...
async function handleBalance(userId, env) {
  ...
  return Response.json({ balance: user?.tokens || 0, usd_rate: rate }, { headers: { "Cache-Control": "no-store" } });
}
...
async function handleAuditCheck(req, env) {
  try {
    ...
    `).all();
    let failedBalanceTransactions = [];
    let doubleEntryViolations = [];
    try {
      ...
      failedBalanceTransactions = failedRes?.results || [];
      ...
      doubleEntryViolations = violationsRes?.results || [];
    } catch {
    }
    return Response.json({
      status: "success",
...
async function handleAdminReverseBalance(req, env) {
  try {
    ...
    return Response.json({ ok: true, tx_id: res.txId, user_balance_after: res.userBalanceAfterTenths ? Number(res.userBalanceAfterTenths) / 10 : void 0 });
  } catch (e) {
    return Response.json({ ok: false, error: e.message }, { status: 500 });
  }
}
__name(handleAdminReverseBalance, "handleAdminReverseBalance");
async function handleAdminBalanceStressTest(req, env) {
  try {
    const body = await req.json().catch(() => ({}));
    ...
    const runOne = /* @__PURE__ */ __name(async (i) => {
    ...
    const worker = /* @__PURE__ */ __name(async () => {
      while (idx < total) {
        const i = idx++;
        const r = await runOne(i).catch((e) => ({ ok: false, status: 500, error: String(e?.message || e) }));
        results.push(r);
      }
    }, "worker");
...
__name(handleAdminBalanceStressTest, "handleAdminBalanceStressTest");
    ...
    if (paypalStatus === "APPROVED") {
      ...
      if (captureData.status === "COMPLETED") {
        ...
        if (updateRes.meta.changes > 0) {
          if (tokensAdded > 0) {
            await addUserTokens(transaction.user_id, tokensAdded, env, { logLabel: isSubscription ? "Bonus langganan" : "Top-up" });
          }
          if (isSubscription) {
      ...
      if (updateRes.meta.changes > 0) {
        if (isSubscription) {
          ...
          await env.DB.prepare("UPDATE users SET subscription_active = 1, subscription_expiry = ? WHERE id = ?").bind(newExpiryIso, transaction.user_id).run();
          if (tokensAdded > 0) {
            await addUserTokens(transaction.user_id, tokensAdded, env, { logLabel: "Bonus langganan" });
          }
          const userEmailRow = await env.DB.prepare("SELECT email FROM users WHERE id = ?").bind(transaction.user_id).first();
          if (userEmailRow && userEmailRow.email) {
            const emailSubject = `Subscription Activated (${durationDays} Days)`;
        ...
        await addUserTokens(transaction.user_id, tokensAdded, env, { logLabel: "Top-up" });
      }
      return Response.json({
        status: "paid",
        paypal_status: "COMPLETED",
      ...
      if (transaction && transaction.status === "pending") {
        ...
        if (transaction.method === "paypal_subscription") {
          ...
          if (transaction.tokens_added && transaction.tokens_added > 0) {
            await addUserTokens(transaction.user_id, transaction.tokens_added, env, { logLabel: "Bonus langganan" });
          }
        ...
        const newBalance = await addUserTokens(transaction.user_id, transaction.tokens_added, env, { logLabel: "Top-up" });
...
init_modules_watch_stub();
function isVisionLikeModel(providerRaw, modelRaw) {
  ...
  return model.includes("vision") || model.includes("/vl") || model.includes("-vl") || model.includes("pixtral") || model.includes("gpt-4o") || model.includes("gemini") || model.includes("claude-3") || model.includes("llava") || model.includes("internvl");
}
__name(isVisionLikeModel, "isVisionLikeModel");
function filterVisionModelPrices(rows) {
...
__name(filterVisionModelPrices, "filterVisionModelPrices");
function summarizeModelRows(rows) {
  const list = Array.isArray(rows) ? rows : [];
  const byProvider = {};
  for (const r of list) {
...
__name(summarizeModelRows, "summarizeModelRows");
...
var src_default = {
  ...
  async fetch(request, env, _ctx) {
    ...
    const corsHeaders = {
      ...
      "Access-Control-Allow-Headers": "Content-Type, Authorization, x-admin-key, x-debug-model-filter"
    };
    ...
    try {
      ...
      if (path === "/config/models" && method === "GET") {
        try {
          ...
          const debug = request.headers.get("x-debug-model-filter") === "1" || new URL(request.url).searchParams.get("debug") === "1";
          const filtered = filterVisionModelPrices(results);
          if (debug) {
            const before = summarizeModelRows(results);
            const after = summarizeModelRows(filtered);
            return await wrapCors(Promise.resolve(new Response(JSON.stringify({
              success: true,
              data: filtered,
              meta: { before, after }
          ...
          return await wrapCors(Promise.resolve(new Response(JSON.stringify({ success: true, data: filtered }), { headers: { ...corsHeaders, "Cache-Control": "no-store" } })));
        } catch (e) {
