...
// .wrangler/tmp/bundle-cjZQA6/checked-fetch.js
...
async function handleUpdateSubscription(request, env) {
  ...
  try {
    ...
    await env.DB.prepare("UPDATE users SET subscription_active = ?, subscription_expiry = ? WHERE id = ?").bind(activeVal, expiry_date, user_id).run();
    return Response.json({ success: true });
  } catch (e) {
    return Response.json({ error: e.message }, { status: 500 });
  }
...
// .wrangler/tmp/bundle-cjZQA6/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-cjZQA6/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
