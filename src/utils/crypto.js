...
export async function encryptApiKey(key, secret) {
    ...
    const encrypted = await window.crypto.subtle.encrypt({
        name: "AES-GCM",
        iv: iv
    }, cryptoKey, enc.encode(key));
    return {
        iv: arrayBufferToBase64(iv.buffer),
        data: arrayBufferToBase64(encrypted)
    };
}
export async function decryptApiKey(encryptedData, ivStr) {
    ...
    }, cryptoKey, base64ToArrayBuffer(encryptedData));
